    ...
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="px-6 pb-6 overflow-y-auto custom-scrollbar flex-1">
              ...
              <div className="space-y-2">
                ...
                {order.customer_phone && (
                  <div className="flex items-center gap-1.5">
                    <Phone size={9} className="text-emerald-600 shrink-0" />
                ...
                <div className="pt-2 border-t border-dashed border-slate-200 flex items-start gap-1.5">
                  <MapPin size={10} className="text-emerald-600 mt-0.5 shrink-0" />
                  <p className="text-[9px] font-bold text-slate-500 leading-relaxed uppercase">
                    {order.is_pickup
                      ? "AMBIL DI TOKO (UD BAROKAH)"
          ...
          <div className={`grid grid-cols-2 gap-3 mb-4 transition-all duration-500 delay-[250ms] ${animate ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
          ...
          {order.notes && (
            <div className={`p-4 rounded-2xl bg-amber-50 border border-amber-100 mb-4 transition-all duration-500 delay-[300ms] ${animate ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
              <span className="text-[8px] font-black text-amber-600 uppercase tracking-widest flex items-center gap-1.5 mb-1">
                <FileText size={11} /> Catatan Pesanan
              </span>
                  ...
                  {order.items?.map((item, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/30 transition-colors">
                      ...
                      <td className="px-4 py-2.5 text-center">
                        <span className="text-[9px] font-black text-slate-700 bg-slate-100 px-2 py-0.5 rounded-lg">
        ...
        <div className={`px-6 py-4 bg-slate-50/50 border-t border-slate-100 flex items-center justify-between gap-3 shrink-0 transition-all duration-500 delay-[450ms] ${animate ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
          <p className="hidden sm:block text-[8px] font-bold text-slate-400 uppercase tracking-widest">
            UD Barokah - Sistem Distribusi
          </p>
          <button
            onClick={handleClose}
            className="px-6 py-2 bg-slate-900 text-white text-[9px] font-black uppercase tracking-widest rounded-lg hover:bg-black transition-all active:scale-95 shadow-lg"
          >
            Tutup
          </button>
        </div>
