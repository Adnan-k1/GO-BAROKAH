  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] overflow-hidden font-sans">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div
          className="flex-shrink-0 bg-[#F8FAFC] transition-all duration-500 ease-in-out overflow-hidden"
          style={{ maxHeight: isScrolled ? "90px" : "450px" }}
        >
          <div className="flex items-center justify-between px-8 pt-8">
            <div
              className={`transition-all duration-500 ${isScrolled ? "opacity-0 -translate-y-10" : "opacity-100 translate-y-0"}`}
            >
              <h1 className="text-xl font-black text-slate-900 tracking-tight uppercase">
                Katalog Produk
              </h1>
              <p className="text-[10px] font-bold text-slate-400 mt-0.5 uppercase tracking-[0.2em]">
                Sistem Inventaris UD BAROKAH
              </p>
            </div>
            <button
              onClick={() => {
              ...
              className="flex items-center gap-2 bg-[#1a4d2e] text-white px-5 py-3 rounded-xl text-[10px] font-black uppercase shadow-lg active:scale-95 transition-all"
            >
              <Plus size={14} strokeWidth={3} />
              <span className={isScrolled ? "hidden" : "block"}>
                Produk Baru
              </span>
            </button>
          </div>
          ...
          <div
            className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 px-8 py-6 transition-all duration-500 ${isScrolled ? "opacity-0 scale-95 pointer-events-none -mb-[140px]" : "opacity-100 scale-100 mb-0"}`}
          >
            <InventoryStatCard
              label="Varian"
            ...
            <InventoryStatCard
              label="Kosong"
              value={products.filter((p) => (Number(p.stock) || 0) <= 0).length}
          ...
          <div
            className={`px-8 py-2 transition-all duration-500 ${isScrolled ? "-translate-y-10" : "translate-y-0"}`}
          >
            <ProductFilterBar
              search={search}
            ...
            >
              <table className="w-full border-collapse min-h-full">
                <thead className="bg-slate-50/50 sticky top-0 backdrop-blur-md z-10 border-b border-slate-100">
                  <tr>
                    {["Produk", "Kategori", "Stok", "Harga", "Aksi"].map(
                      (h) => (
                        <th
                          key={h}
                          className="px-8 py-4 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] text-left"
                        >
                          {h}
                        </th>
                      ),
                  ...
                  {isLoading ? (
                    ...
                    <>
                      {paginatedItems.map((p) => (
                        <tr
                          key={p.id}
                          className="hover:bg-slate-50/50 h-[73px]"
                        >
                          ...
                          <td className="px-8 py-4">
                            <div className="flex flex-col">
                              <span
                                className={`font-black text-xs ${Number(p.stock) <= 5 ? "text-red-500" : "text-slate-700"}`}
                              >
                                {p.stock}
                              </span>
                          ...
                          <td className="px-8 py-4">
                            <div className="flex gap-2">
                              <button
                                ...
                                className="p-2.5 bg-slate-50 text-slate-400 hover:text-emerald-600 rounded-xl active:scale-90 transition-all"
                              >
                                <Pencil size={14} />
                              </button>
                              <button
                                ...
                                className="p-2.5 bg-slate-50 text-slate-400 hover:text-red-500 rounded-xl active:scale-90 transition-all"
                              >
                                <Trash2 size={14} />
                              </button>
                            </div>
                      ...
                      {paginatedItems.length < PER_PAGE && (
                        <tr
                          style={{
                            height: `${(PER_PAGE - paginatedItems.length) * 73}px`,
                          }}
                        >
                          <td colSpan={5}></td>
                        </tr>
                    ...
                    <tr>
                      <td
                        colSpan={5}
                        className="py-24 text-center uppercase tracking-widest text-slate-300 font-black text-[10px]"
                      >
                        Kosong
                      </td>
                    </tr>
            ...
            <footer className="px-8 py-4 border-t border-slate-50 flex items-center justify-between bg-white flex-shrink-0">
              ...
              <div className="flex gap-1.5">
                <button
                  ...
                  className="p-2.5 rounded-xl border border-slate-100 hover:bg-slate-50 disabled:opacity-20 active:scale-95 transition-all"
                >
                  <ChevronLeft size={16} />
                </button>
                <button
      ...
      {modalMode === "delete" && (
        <ConfirmModal
          ...
          onClose={() => setModalMode(null)}
          onConfirm={async () => {
            const success = await handleDelete(selected.id);
