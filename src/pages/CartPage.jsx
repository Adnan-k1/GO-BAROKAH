  ...
  <div className="group relative flex flex-col sm:flex-row items-center gap-6 bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm transition-shadow duration-200 hover:shadow-md">
    ...
    <div className="flex-grow text-center sm:text-left">
      <span className="text-[#2D5A43] font-black text-[9px] uppercase tracking-[0.2em] opacity-60">
        {item.category || 'Barokah Choice'}
      </span>
      <h3 className="text-lg font-black text-gray-900 tracking-tight mt-1">{item.name}</h3>
    ...
    <div className="flex items-center gap-3">
      <div className="flex items-center gap-4 bg-gray-50 rounded-2xl p-2 border border-gray-100">
        <button 
          onClick={onDecrement} 
          className="w-10 h-10 flex items-center justify-center bg-white rounded-xl text-gray-400 hover:text-red-500 transition-colors active:scale-95"
        >
          <Minus size={16} />
        </button>
        ...
        <button 
          onClick={onIncrement} 
          className="w-10 h-10 flex items-center justify-center bg-white rounded-xl text-gray-400 hover:text-[#2D5A43] transition-colors active:scale-95"
        >
          <Plus size={16} />
        </button>
      </div>
...
const OrderSummary = ({ subtotal, total, onCheckout }) => (
  <div className="bg-white rounded-[2.5rem] p-10 border border-gray-100 shadow-xl shadow-gray-200/40 flex flex-col">
    ...
    <button
      onClick={onCheckout}
      className="w-full bg-[#2D5A43] text-white py-6 rounded-2xl font-black text-xs uppercase tracking-[0.2em] flex items-center justify-center gap-3 hover:bg-[#234735] transition-all active:scale-95 group"
    >
...
const EmptyCartView = () => (
  <div className="flex flex-col items-center justify-center py-40 bg-white min-h-screen">
    <div className="bg-[#FBFBFB] w-32 h-32 rounded-[2.5rem] flex items-center justify-center mb-8 text-gray-200">
    ...
    <Link
      to="/store"
      className="bg-[#2D5A43] text-white px-12 py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] hover:bg-[#234735] transition-all active:scale-95"
    >
      Mulai Belanja
    </Link>
  </div>
